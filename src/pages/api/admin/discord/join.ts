...
export const POST: APIRoute = async ({ locals, request }) => {
  ...
  if (!studentUser.discordAccessToken) {
    return Response.json({ error: 'El alumno no ha otorgado permisos de Discord. Debe cerrar sesión y volver a entrar.' }, { status: 400 })
  }

  try {
    const response = await addMemberToGuild(
    ...
    if (!response.success) {
      return Response.json({ error: response.error || 'Error al añadir al servidor. Revisa los permisos del bot.' }, { status: 500 })
    }
